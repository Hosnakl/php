...
function ValidatePrincipal($amount) {
    ...
    if (empty($amount)) {
        $GLOBALS['error'] = true;
        return $GLOBALS['amount_error'] = "Principal amount can not be blank";
    } else {
        if ($amount <= 0 || !is_numeric($amount)) {
            $GLOBALS['error'] = true;
            return $GLOBALS['amount_error'] = "Prioncipal amount must be numeric";
        } else {
...
<div class="container">
    <h2 class="text-center m-4">Deposit Calculator</h2>
...
<?php $amount_error ? print("<P class='error text-danger'>$amount_error</p>") : "" ?>
            </div>
...
<?php $rate_error ? print("<P class='error text-danger'>$rate_error</p>") : "" ?>
